...
To develop bitcore-node-monacoCoin:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-monacoCoin.git
...
We now will setup symlinks in `bitcore-node-monacoCoin` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-monacoCoin
ln -s ~/bitcoind-rpc-monacoCoin
```
...
cd devnode
mkdir node_modules
touch bitcore-node-monacoCoin.json
touch package.json
```

Edit `bitcore-node-monacoCoin.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-monacoCoin/bin/bitcore-node-monacoCoin start
```